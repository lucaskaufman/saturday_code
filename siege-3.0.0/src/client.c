  ...
  if ((my.verbose && !my.get) && (!my.debug)) {
    ...
    if (my.csv) {
      if (my.display)
        ...
        etime, bytes, (my.fullurl)?url_get_absolute(U):url_get_request(U), url_get_ID(U), fmtime
      );
      else
      ...
      if (my.display)
        DISPLAY(
          ...
          (my.fullurl)?url_get_absolute(U):url_get_request(U)
        ); 
      else
        DISPLAY ( 
  ...
  switch (head->code) {
    ...
    case 307:
      if (my.follow && head->redirect[0]) {
        redirect_url = new_url(head->redirect);
